...
// nextInt as provided by Random is exclusive of the top value so you need to add 1
        ...
        startpayment.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                {
                    Toast.makeText(paytmActivity.this, "Amount is empty", Toast.LENGTH_LONG).show();
                }else {
                    startPayment();
                }
            }
    ...
    private void startPayment() {
       ...
       /* ApiClient service = ServiceGenerator.createService(ApiClient.class);
        ...
        Call<Token_Res> call=service.generateTokenCall("1234",Bankapi,orderIdString,payment);
      call.enqueue(new Callback<Token_Res>() {
          @Override
          public void onResponse(Call<Token_Res> call, Response<Token_Res> response) {
              try {
                ...
                if(status1.equals("TXN_SUCCESS"))
                {
                    try {
                        ...
                        if (mSchemeType.equals("1")) {
                            ...
                            loadScheme(String.valueOf(txnId));
                        } else if(mSchemeType.equals("2") ){
                            loadGramScheme(String.valueOf(txnId));
                            // loadScheme(s);;
                        }else
                        {
                            loadCustomizeScheme(String.valueOf(String.valueOf(txnId)));
                        }
                    }
                    catch (Exception e)
                    {
        ...
        if (requestCode == ActivityRequestCode && data != null) {
            ...
            try {
                ...
                if (mSchemeType.equals("1")) {
                    ...
                    loadScheme(String.valueOf(requestCode));
                } else if(mSchemeType.equals("2") ){
                    loadGramScheme(String.valueOf(requestCode));
                    // loadScheme(s);;
                }
                else
                {
                    loadCustomizeScheme(String.valueOf(requestCode));
                }
            }
            catch (Exception e)
            {
    ...
    private void loadCustomizeScheme(String s) {
        ...
        call.enqueue(new Callback<List<DataStatus>>() {
            ...
            public void onFailure(Call<List<DataStatus>> call, Throwable t) {
                mProgressdialog.cancel();
                if(getContext() !=null) {
                    mInfo= Responseinfodialog.alertshow(paytmActivity.this, "CONNECTION FAILURE", Color.RED);
                    mInfo.show();
                }
                t.printStackTrace();
